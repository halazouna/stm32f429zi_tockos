...
pub fn get_oc_value(timer: Timer, oc_id: OutputCompareChannel) -> u32{
    match oc_id {
        OutputCompareChannel::OChannel1 => timer.ccr1.get(),
        OutputCompareChannel::OChannel2 => timer.ccr2.get(),
        OutputCompareChannel::OChannel3 => timer.ccr3.get(),
        OutputCompareChannel::OChannel4 => timer.ccr4.get(),
        _ => 0
    }
}
...
// TIM_ALARM = TIMER5
pub static mut TIM_ALARM: TimerAlarm = TimerAlarm {
    timer: TIM5,
    client: Cell::new(None),
};

// pub static mut TIM_2: Timer = Timer {
//     timer: TIM2,
//     client: Cell::new(None),
// }

// pub static mut TIM_3: Timer = Timer {
//     timer: TIM3,
//     client: Cell::new(None),
// }
...
impl TimerAlarm {
    ...
    pub const fn new(timer: Timer)-> TimerAlarm {
        TimerAlarm {
            timer: timer,
            client: Cell::new(None),
        }
    ...
    pub fn set_client<C: hil::time::Client>(&self, client: &'static C) {
        self.client.set(Some(client));
    }

    pub fn clear_alarm(&self) {
        set_counter(self.timer, 0);
    ...
    pub fn start(&self) {
        // Enable Timer clock
        match self.timer {
            TIM2   => rcc::enable(rcc::Peripheral::TIM2),
            TIM3   => rcc::enable(rcc::Peripheral::TIM3),
            TIM5   => rcc::enable(rcc::Peripheral::TIM5),
            _ => { }
        };
        ...
        set_mode(self.timer, flags::cr1::CKD_CK_INT, flags::cr1::CMS_MASK, flags::cr1::DIR_UP);

        // Set auto-reload value
        set_period(self.timer, 0xFFFFFFFF);

        // Set prescaler value
        set_prescaler(self.timer, 15);

        //  //
        // Enable OC channel 1 and 2
        enable_oc_output(self.timer, OutputCompareChannel::OChannel1);
        enable_oc_output(self.timer, OutputCompareChannel::OChannel2);

        // Set Output Compare value
        let mut PreviousVal = get_counter(self.timer);
        set_oc_value(self.timer, OutputCompareChannel::OChannel2, PreviousVal + 1000);

        // Enable IRQ
        enable_irq(self.timer, flags::dier::CC2IE);
    }
    ...
    pub fn disable_interrupt(&self) {
        disable_irq(self.timer, flags::dier::CC2IE);
    }
}
...
impl hil::time::Time for TimerAlarm {
    ...
    fn is_armed(&self) -> bool {
        ((self.timer.dier.get() & flags::dier::CC2IE) >> 2) > 0
    }
} 
...
impl hil::time::Alarm for TimerAlarm {
    ...
    fn set_alarm(&self, tics: u32) {
        ...
        // Set new value
        set_oc_value(self.timer, OutputCompareChannel::OChannel2, tics + 1000);
    ...
    fn get_alarm(&self) -> u32 {
        get_oc_value(self.timer, OutputCompareChannel::OChannel2)
    }
}